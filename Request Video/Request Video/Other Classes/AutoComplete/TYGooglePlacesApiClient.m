...
NSString *const apiKey = @"AIzaSyAuD0eUx__zzhxd1ND0eplTD0J6Wx50wg0";
//25 W San Fernando St, San Jose, CA 95113, USA
//1855 S Delaware St, San Mateo, CA 94402, USA
//106 Court St, Middletown, CT 06457, USA
...
-(void)retrieveGooglePlaceInformation:(NSString *)searchWord ForLocation:(CLLocation *)location withCompletion:(void (^)(BOOL isSuccess, NSError *error))completion {
    ...
    if ([self.searchResultsCache objectForKey:searchWord]) {
        ...
        NSString *urlString = [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/autocomplete/json?input=%@&types=establishment|geocode&location=%f,%f&radius=500&language=en&key=%@",searchWord,location.coordinate.latitude,location.coordinate.longitude,apiKey];
